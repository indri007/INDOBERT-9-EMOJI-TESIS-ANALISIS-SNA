...
| Rank | User Handle | Total Degree | Degree Centrality | In-Degree ($k^{\text{in}}$) | Out-Degree ($k^{\text{out}}$) | Betweenness ($C_B$) | Communicative Role Classification |
|:---:|:---|:---:|:---:|:---:|:---:|:---:|:---|
...
### 7.3 Research Implementation Roadmap (2026–2029)

Beyond the five theoretical research pillars articulated above, this study is embedded within a living, iterative research program with a concrete **three-phase implementation roadmap** targeting the progressive operationalization of findings into functional policy intelligence infrastructure.

#### Phase I: Corpus Expansion and Model Hardening (Q3–Q4 2026)

The immediate priority is to resolve the pipeline anomaly documented in Section 4.7 and expand the empirical corpus temporally and thematically.

**Target Milestones:**
- **Corpus Re-annotation**: Deploy three independent annotators (two native Indonesian NLP practitioners and one political communication expert) to re-label the 561-tweet held-out test set using the standardized 9-class emotion taxonomy. Target: Krippendorff's $\alpha \geq 0.82$ before releasing benchmark scores.
- **Corpus Extension**: Extend data collection through the full MBG implementation lifecycle (June–December 2026), targeting a corpus expansion to $N \geq 15,000$ tweets using both X API v2 filtered stream endpoints and Brand24's Social Media Monitoring API, which provides cross-platform keyword tracking across X, news portals, YouTube, and web forums simultaneously.
- **Emoji-Aware Augmentation**: Construct a **Sarcasm-Emoji Paired Augmentation Dataset** of 2,000 synthetically constructed tweet pairs (literal baseline + emoji-inverted sarcastic variant) using back-translation and controlled prompting to supplement fine-tuning.
- **Model Release**: Publish fine-tuned `indobert-mbg-sarcasm-v2` as an open-access Hugging Face model card with evaluation benchmarks, inference code, and a public API endpoint for community validation.

#### Phase II: Real-Time Monitoring Dashboard and DPPTI Operationalization (Q1–Q2 2027)

The second phase operationalizes the research findings into a functional, deployable government intelligence dashboard.

**Architecture Design:**

The proposed **MBG Policy Intelligence Dashboard** is structured as a three-tier computational stack:

1. **Data Ingestion Layer**: Continuous data ingestion via (a) Social media monitoring API providing real-time cross-platform mention streams covering X, online news portals, YouTube, and community web forums; (b) X API v2 filtered stream capturing Boolean keyword clusters; and (c) BGN's forthcoming open data REST API endpoints for operational SPPG status. Target ingestion rate: $\geq 500$ mentions/hour during active program phases.

2. **Analytics Processing Layer**: Real-time analytics executed through:
   - **Emotion Classification Engine**: Fine-tuned IndoBERT inference pipeline (latency target: $< 200$ ms/tweet) classifying each incoming mention across the 9-class affective taxonomy.
   - **Sarcasm Detection Filter**: Binary emoji-aware classifier flagging pragmatically inverted tweets for escalated monitoring priority.
   - **ABSA Dispatcher**: Aspect classifier routing each mention to the $A_1$ (Budget), $A_2$ (Logistics), or $A_3$ (Nutritional Quality) policy dimension.
   - **DPPTI Calculator**: Continuous computation of $\text{DPPTI}_t$ refreshed every 15 minutes using the formal composite trust index formula.
   - **Network Topology Tracker**: Incremental graph updates via NetworkX streaming additions, recomputing Louvain modularity, in-degree centrality, and dyadic reciprocity on a weekly rolling basis.

3. **Visualization and Alerting Layer**: Deployed as a Streamlit multi-page web application featuring:
   - **Real-Time Sentiment Heatmap**: Choropleth map of Indonesian provinces colored by ABSA disgust concentration, sourced from geotagged mention metadata.
   - **DPPTI Trend Line**: 30-day rolling visualization of the composite trust index with threshold alert triggers ($\text{DPPTI}_t < 0.20$ triggers Orange Alert; $\text{DPPTI}_t < 0.10$ triggers Red Crisis Alert).
   - **Network Evolution Monitor**: Weekly force-directed graph snapshots rendered through Pyvis, tracking centrality shifts and community dissolution/formation.
   - **Sarcasm Spike Detector**: Hourly sarcasm rate per policy aspect, with automatic WhatsApp/Telegram notification to Humas BGN when rates exceed 2 standard deviations above the 7-day rolling mean.
   - **AI Oracle Query Intelligence Module**: Semantic clustering of citizen queries directed at platform AI entities, providing BGN with a live diagnostic window into unresolved citizen epistemic anxieties.

**Target Deployment**: Pilot launch with BGN's Bureau of Legal Affairs and Public Relations (Biro Hukum dan Humas BGN) for a 90-day operational evaluation period beginning Q2 2027.

#### Phase III: Sovereign AI Fact-Checking Infrastructure and Open Policy Data (Q3 2027–Q4 2028)

The third phase addresses the structural root cause of algorithmic epistemic displacement by constructing sovereign, government-owned fact-checking infrastructure.

**Key Initiatives:**

1. **@BGNVerifikasi Bot Development**: Develop and deploy an official, verified AI fact-checking assistant on platform X. The bot will: (a) monitor all incoming queries directed at platform AI entities regarding MBG budget, SPPG status, and food safety incidents; (b) retrieve verified answers from BGN's machine-readable open data API endpoints established in partnership with the Ministry of Finance; (c) respond within 60 seconds in clear, empathetic, colloquial Indonesian citing official data sources; and (d) automatically escalate queries containing viral misinformation patterns to human communication officers.

2. **BGN Open Data API Launch**: In coordination with the Ministry of Finance (Kemenkeu) and the Government Financial Management Information System (SAKTI), establish a publicly accessible REST API suite providing: real-time per-SPPG disbursement data (unit ID, regency, active/suspended status, last audit date); per-beneficiary cost data updated monthly; laboratory food safety certificates and inspection logs for every active SPPG; all data serialized in JSON-LD format conforming to the Indonesian National Government Open Data Standard (SNDI).

3. **Academic-Government Data Partnership (MoU Framework)**: Formalize a Memorandum of Understanding between UPN "Veteran" Jawa Timur, BGN, and Kemkomdigi establishing a **Living Research Consortium** for continuous MBG discourse monitoring. This MoU will: (a) grant academic researchers access to anonymized BGN operational databases for longitudinal comparative analysis; (b) establish a quarterly reporting cycle providing BGN with computational sentiment intelligence reports; and (c) fund the operational maintenance of the DPPTI dashboard under a joint national research grant framework (Hibah Penelitian Nasional, Kemendikbudristek).

---

### 7.4 Theoretical Contributions and Knowledge Advancement

Beyond immediate policy applications, this research advances the theoretical frontiers of three intersecting academic disciplines:

#### 7.4.1 Computational Communication Science
This study introduces the **Pragmatic Emoji Incongruence (PEI) framework** as a formalizable, computationally operationalizable construct for identifying digital sarcasm. By treating emoji tokens as illocutionary force indicators (Dresner & Herring, 2010) within a multi-task IndoBERT architecture, the study provides a replicable blueprint for paralinguistic sentiment analysis applicable to Indonesian, Malay, Filipino, Thai, and other Southeast Asian digital communication contexts where emoji-saturated political discourse is ubiquitous.
...
## Appendix A: Formal Mathematical Notations and Centrality Formulas
...
**Table C1: Inductive Semantic Lexicon and Indicator Keyword Schema for ABSA Classification**
...
This observation suggests that citizens treat platform-integrated AI as a substitute for investigative journalism and judicial inquiry, cementing its role as the highest in-degree AI actor in the observed network in the social network.
