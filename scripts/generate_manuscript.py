...
Previous studies on Indonesian digital policy discourse exhibit four critical limitations:
1. **Binary Reductionism:** Most empirical studies rely on lexicon-based or basic machine learning classifiers (SVM, Naïve Bayes, LSTM) restricted to binary (positive/negative) or 3-class sentiment (Riza & Charibaldi, 2021; Saputri et al., 2018). These models obscure granular emotional nuances—such as the vital distinction between *Anger* (normative injustice) and *Disgust* (visceral rejection of physical unhygienic conditions).
2. **Methodological Isolation:** NLP sentiment analyses are almost universally conducted in isolation from Social Network Analysis (SNA). Consequently, researchers observe *what* is said without knowing *who* occupies structural power, or they map *who interacts with whom* without understanding the affective states animating the network clusters (Suaib & Pratiwi, 2025; Sulafasyah, 2026).
3. **Absence of Pragmatic Sarcasm Modeling:** Digital communication research often treats sarcasm as statistical noise rather than an organized rhetorical counter-discourse rooted in pragmatic pretense (Grice, 1975; Clark & Gerrig, 1984).
4. **Theoretical Decoupling:** Existing public relations and crisis communication models (e.g., Coombs' SCCT) have struggled to account for the hybrid physical-digital interface of modern social welfare policies. There is a lack of theoretical bridging between marketing frameworks of brand immersion and public policy crisis management.

### 1.2 Theoretical and Methodological Novelty
To resolve this knowledge gap, this study introduces a unified, tri-layer computational communication framework. Theoretically, we bridge Philip Kotler, Hermawan Kartajaya, and Iwan Setiawan's (2023) concept of the **Phygital Gap** (from Marketing 6.0) into the domain of public administration and political communication. The *Phygital Gap* posits that public dissatisfaction is triggered not by ideological rejection of a policy's digital vision, but by an intolerable chasm between digital promises (*online narrative*) and sensory physical reality (*offline operational delivery*).

Methodologically, this paper integrates:
- A fine-tuned, bidirectional transformer language model (`indobenchmark/indobert-base-p2`, Wilie et al., 2020) customized for 9 Plutchik-derived emotion classes.
- An algorithmic rule-based sarcasm detector operationalizing lexical oppositions and text-emoji incongruity.
- A directed graph topological analysis using NetworkX and the Louvain community detection algorithm (Blondel et al., 2008).
- Thematic Aspect-Based Sentiment Analysis (ABSA) verifying affective concentration across physical operational pillars.
